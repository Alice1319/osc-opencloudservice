...
import org.eclipse.xpanse.orchestrator.plugin.huaweicloud.enums.BuilderState;
...
public abstract class AtomBuilder {
    ...
    public static ExecutorService executorService = new ThreadPoolExecutor(CPU_COUNT + 1,
        CPU_COUNT * 10, 300, TimeUnit.SECONDS, new LinkedBlockingDeque<>(100));
    ...
    public boolean build(BuilderContext ctx) {
        ...
        executorService.execute(() -> {
            Thread.currentThread()
                .setName("BuildThread_" + this.name() + "_" + System.currentTimeMillis());
     ...
     *
     * @param ctx            The building context for the whole building progress.
     * @param subBuilderList Create all the Builders asynchronously.
     */
    ...
    private void rollbackAfterBuildFailed(BuilderContext ctx) {
        ...
        AtomBuilder basicBuilder = ctx.getBuilderMap().get(BuilderFactory.BASIC_BUILDER);
